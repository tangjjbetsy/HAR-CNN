...
def train(optimizer, criterion, net, validation, device, epoches, save_path=SAVEPATH):
    iterations = 0
    start = time.time()
    ...
    if validation:
        train_loader, dev_loader = data_loader("train", True, validation)
    else:
        train_loader = data_loader("train", True, validation)
